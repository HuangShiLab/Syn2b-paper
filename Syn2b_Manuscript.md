...
bias that grows with the number of contigs. On 43,334 held-out pairs from
GTDB-R207, Syn2b's inverted aligned fraction agrees with the alignment-based
truth at Pearson r = 0.94 overall and r = 0.996 at ≥97% ANIm, with slope ≈ 1
and intercept ≈ 0 across divergence bands. A multi-enzyme panel
...
set (Figure 3a). Conditioning on measured ANIm shows the
...
At ≥97% ANIm (n = 3,826), the correlation is r = 0.9960 (95% CI 0.996–0.996),
slope = 1.006, intercept = −0.004, with SD(error) = 0.0135 (Figure 3b). Bland–Altman mean
...
*Helicobacter pylori* isolates (source and accessions listed in Supplementary
Table 3). popANI ranged from 0.270 to 1.000 (median 0.538), whereas Syn2b
synteny ranged from 0.819 to 1.000 (median 0.963). The two metrics were only
modestly correlated (Pearson r = 0.50, 95% CI 0.47–0.53), consistent with the
*H. pylori*-like / *N. gonorrhoeae*-like regime in which recombination decouples
SNP similarity from synteny. Within-host pairs (n = 456) showed the same pattern
as between-host pairs, indicating that structural variation is pervasive even at
...
Pairs were drawn from the GTDB-R207 representative genome set. The 43,334 pairs
constitute a held-out validation set independent of the training data used for
the companion Syn2bANI tool; pairs were stratified across the ANI range
observed in GTDB-R207. For each pair, ANIm was computed with minimap2
(`-cx asm20`). dnadiff (MUMmer 4.0) was run with default parameters; the
`.1coords` output was used to derive the inverted aligned fraction, and the
`.report` output was used for breakpoint and structural-event counts. Syn2b was
run with the BcgI+AlfI+BplI+CjePI multi-enzyme panel, reporting both
majority-frame and fixed-reference inverted fractions. Pearson and Spearman
correlations, linear regressions, and partial correlations (controlling for
ANIm and contig count) were computed in Python with scipy and statsmodels.
Confidence intervals for Pearson correlations were obtained by Fisher
z-transformation.
