...
(ANIm ≥ 97%, n = 3,826) Syn2b's fixed-reference inverted aligned fraction agrees
with alignment-based truth at Pearson r = 0.996, slope = 1.006; across 43,312
held-out GTDB-R207 pairs, r = 0.94. A four-enzyme panel (BcgI+AlfI+AloI+FalI)
yields ~1.34 tags/kb in *E. coli* K-12 and detects inversions down to ~4 kb.
Applied genome-wide, we find that 34% of GTDB-R207 pairs with ANIm ≥ 97% carry
≥2 alignment-visible inversion events — rearrangements invisible to ANI
ranking. Syn2b is available at https://github.com/HuangShiLab/Syn2b.
...
(pooled statistics below and in section 6 refer to this sample). Figure 3b shows the same trend with
...
alignment fragmentation at 85–90% ANIm. The contamination is directly visible:
across the held-out set, dnadiff's breakpoint count correlates with assembly
contig count at Spearman ρ = 0.28 (partial ρ = 0.34 controlling ANIm), and
62% of `synteny_blocks` are contig starts (correlation with contig count
r = 0.771). Syn2b's junction count, once the same contig correction is applied,
shows no residual contig-count dependence. These results support the
fragmentation principle and argue for prioritizing length-weighted ratios in
structural comparison of draft assemblies.
...
dnadiff inversions between them (top cases in Supplementary Table 6).
...
18.1 h on 16 cores (135 s per ordered pair; Supplementary Table 4), about
...
gives a concrete use case. GTDB-R207 contains 1,295 genome pairs that any
ANI-only search will rank as near-identical (ANIm ≥ 97%) yet that carry ≥2
alignment-visible inversions. Fast adjacency-based metrics make such pairs
...
partial correlation controlling for ANIm.
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 2 | 4 | 515.7 | 167.7 | 19.63 | 19.80 | 4,467.0 | 38x | 8,663x |
| 5 | 25 | 39.8 | 14.8 | 4.05 | 4.05 | 1,871.0 | 102x | 46,971x |
| 10 | 100 | 23.2 | 6.6 | 5.83 | 5.83 | 863.2 | 251x | 37,171x |
| 15 | 225 | 22.1 | 4.9 | 15.02 | 15.04 | n.d. | 684x | n.d. |
| 22 | 484 | 18.9 | 2.3 | 7.98 | 7.98 | 282.0 | 423x | 14,947x |
...
| dnadiff (MUMmer) | 1,843.9 | 7.979 | no | yes (alignment-based truth) |
| SynTracker (16 cores) | 65,162.0 | 282.086 | no | yes (gene synteny) |
| skani + dnadiff | 1,844.5 | 7.984 | yes | yes |
